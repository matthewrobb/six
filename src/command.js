// The `six` utility. Handles command-line compilation of Six
...
// Allow Six to emit Node.js events.
...
// The list of all the valid option flags that `six` knows how to handle.
var SWITCHES = [
  ['-b', '--bare', 'compile without a top-level function wrapper'],
  ['-c', '--compile', 'compile to JavaScript and save as .js files'],
  ['-e', '--eval', 'pass a string from the command line as input'],
  ['-g', '--global', 'put code directly into global scope rather than UMD define'],
  ['-h', '--help', 'display this help message'],
  ['-i', '--interactive', 'run an interactive Six REPL'],
  ['-j', '--join [FILE]', 'concatenate the source Six before compiling'],
...
// is passed, recursively compile all '.six' extension source files in it
...
var compileScript = function(file, input, base) {
  ...
  try {
    ...
    else if (o.nodes) printLine(six.nodes(t.input, t.options))
...
// Watch a source Six file using `fs.watch`, recompiling it every

var printLine, printWarn, hidden, SWITCHES, opts, sources, sourceCode, notSources, watchers, optionParser, compilePath, compileScript, watch, watchDir, unwatchDir, removeSource, outputPath, writeJs, wait, timeLog, parseOptions, compileOptions, usage, version, fs = require("fs"), path = require("path"), helpers = require("./helpers"), optparse = require("./optparse"), six = require("./six"), child_process = require("child_process"), spawn = child_process.spawn, exec = child_process.exec, EventEmitter = require("events").EventEmitter, uglify = require("uglify-js"), jsp = uglify.parser, pro = uglify.uglify, exists = fs.exists || path.exists;
...
compileScript = function(file, input, base) {
  ...
  try {
    ...
    if (o.tokens) printTokens(six.tokens(t.input)); else if (o.nodes) printLine(six.nodes(t.input).toString().trim()); else if (o.run) six.run(t.input, t.options); else if (o.join && t.file !== o.join) {
      sourceCode[sources.indexOf(t.file)] = t.input;
      compileJoin();
    } else {
      t.output = six.compile(t.input, t.options);
      six.emit("success", task);
      if (o.print) printLine(t.output.trim()); else if (o.compile) writeJs(t.file, t.output, base); else if (o.lint) lint(t.file, t.output);
    }
  } catch (err) {
  ...
  var compile = function() {
    ...
    compileTimeout = wait(25, function() {
      fs.stat(source, function(err, stats) {
        ...
        if (prevStats && stats.size === prevStats.size && stats.mtime.getTime() === prevStats.mtime.getTime()) return rewatch();
        prevStats = stats;
        fs.readFile(source, function(err, code) {
          if (err) return watchErr(err);
...
writeJs = function(source, js, base) {
  ...
  var compile = function() {
    ...
    fs.writeFile(jsPath, js, function(err) {
      if (err) printLine(err.message); else if (opts.compile && opts.watch) timeLog("compiled " + source);
    });
  };
  exists(jsDir, function(itExists) {
    if (itExists) compile(); else exec("mkdir -p " + jsDir, compile);
  });
};

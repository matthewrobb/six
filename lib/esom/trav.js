var query, Traversal, queryPat = /((?:[\>|\+][\s]*)?(?=[\w|\.])(?:[A-Za-z]+)?(?:\.[A-Za-z]+)?(?:\[[\w]+(?:(?:[*|^|$]?=?(?!\]))(?:[0-9]+(?=\]))?(?:['"][\w]+['"](?=\]))?(?:true|false(?=\]))?)?\])*(?:\:(?!$)(?:[\w]+)(?:\((?:[\s\S]+)\))?)*)/g, queryMatch = /^(?:([\>|\+])[\s]*)?(?=[\w|\.])([A-Za-z]+)?(?:\.([A-Za-z]+))?((?:\[[\w]+(?:(?:[*|^|$]?=?(?!\]))(?:[0-9]+(?=\]))?(?:['"][\w]+['"](?=\]))?(?:true|false(?=\]))?)?\])*)((?:\:(?!$)(?:[\w]+)(?:\((?:[\s\S]+)\))?)*)$/, tagPat = "([A-Za-z]+)?", classPat = "(?:.([A-Za-z]+))?", relPat = "(?:([>|+])[s]*)?(?=[w|.])", propPat = "(?:[[w]+(?:(?:[*|^|$]?=?(?!]))(?:[0-9]+(?=]))?(?:['\"][w]+['\"](?=]))?(?:true|false(?=]))?)?])*", pseudoPat = "(?::(?!$)(?:[w]+)(?:((?:[sS]+)))?)*", attrPat = /^([\w]+)(?:(?:([*|^|$]?=)?(?!\]))(?:([0-9]+)(?=\]))?(?:['"]([\w]+)['"](?=\]))?(?:(true|false)(?=\]))?)?\]$/;
